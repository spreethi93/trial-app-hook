...
import { createUpdatableTag } from '@glimmer/reference';
...
export class Meta {
  ...
  addMixin(mixin) {
    assert(this.isMetaDestroyed() ? `Cannot add mixins of \`${toString(mixin)}\` on \`${toString(this.source)}\` call addMixin after it has been destroyed.` : '', !this.isMetaDestroyed());
